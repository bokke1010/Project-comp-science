...
for sharkcount in [4, 12, 20]:
    print(f"Simulating with {sharkcount} sharks")
    for i in range(runs):
        neighbourdata.fill(0)
        sim.simulate(collect_steps * collect_interval, [main.collect_data(collect_range, neighbourdata)], [collect_interval], FOOD_START_COUNT, [FISH_START_COUNT, FISH_START_RADIUS, FISH_START_CHANCE], [], sharkcount)
    ...
    plt.savefig(f"violin-sharkcount={sharkcount}.svg", transparent=True, format = "svg", bbox_inches="tight")
    plt.clf()
